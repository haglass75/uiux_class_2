<template>
  <section class="">
    <!-- 로딩 중 -->
    <div v-if="!job" class="p-4 text-center text-gray-500">
      작업 정보를 불러오는 중...
    </div>

    <!-- 작업 정보 요약 카드 -->
    <div v-else class="bg-gradient-to-br from-blue-600 to-blue-500 rounded-2xl p-5 text-white shadow-lg">
    ...
    <!-- 작업 상태 프로그레스 -->
    <div v-if="job" class="bg-white border border-gray-200 rounded-xl p-4">
      <p class="font-semibold mb-3">작업 진행 상황</p>
      <div class="flex items-center justify-between text-xs text-gray-500 mb-2">
        <span :class="stepClass(1)">예약</span>
        <span :class="stepClass(2)">이동중</span>
        <span :class="stepClass(3)">작업중</span>
        <span :class="stepClass(4)">완료</span>
      </div>
      <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
        <div class="h-full bg-blue-600 rounded-full transition-all" :style="{ width: progressWidth }"></div>
      </div>
    </div>

    <!-- 작업 정보 상세 -->
    <div v-if="job" class="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
      ...
      <div class="grid grid-cols-2 gap-2 text-sm">
        ...
        <div class="bg-green-50 rounded-lg p-3">
          ...
          <p class="font-semibold text-green-700">₩120,000</p>
        </div>
      </div>
    ...
    <!-- 길찾기 및 상태 변경 버튼 -->
    <div v-if="job && job.status !== 'done'" class="bg-white border border-gray-200 rounded-xl p-4">
      <div class="grid grid-cols-2 gap-3">
        ...
        <button
          ...
          <span>↻</span>
          <span>{{ getNextStatusText(job.status) }}</span>
        </button>
      </div>
    </div>
    ...
    <div v-if="job" class="bg-white border border-gray-200 rounded-xl p-4">
      ...
      <div class="grid grid-cols-3 gap-2 mb-3">
        ...
        >
          <img :src="photo" class="w-full h-full object-cover" alt="청소 사진" />
          <button
    ...
    <div v-if="job" class="space-y-3 pb-4">
      <button
        @click="submitWork"
        :disabled="!canSubmit"
        class="w-full py-4 bg-green-600 text-white rounded-xl font-semibold shadow-lg active:scale-[0.98] transition-transform disabled:bg-gray-300 disabled:cursor-not-allowed"
      >
        🎉 작업 완료 및 제출
      </button>
...
import { getJobById, updateJob } from '@/stores/jobsStore'
...
function loadJob() {
  ...
  if (jobId) {
    ...
    if (foundJob) {
      ...
      // 사진 초기화 (저장된 사진이 있으면 표시)
      photos.value = job.value.photos ? [...job.value.photos] : []
      // 서명 초기화
...
const step = computed(() => {
  ...
  if (job.value.status === 'scheduled') return 1
  if (job.value.status === 'onroute') return 2
  if (job.value.status === 'working') return 3
  if (job.value.status === 'done') return 4
  return 1
})
...
function stepClass(n) {
  return n <= step.value ? 'text-blue-600 font-bold' : 'text-gray-400'
}
...
function submitWork() {
  ...
  alert('작업이 완료되었습니다! 정산 내역에서 확인할 수 있습니다.')
...
function changeJobStatus() {
  ...
  if (currentIndex < statusOrder.length - 1) {
    ...
    alert(`작업 상태가 "${statusText(newStatus)}"로 변경되었습니다.`)
  }
}
